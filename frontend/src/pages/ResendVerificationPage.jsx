...
import { Box, Button, TextField, Typography, Paper, Link, InputAdornment } from '@mui/material';
...
const ResendVerificationPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    try {
      const res = await fetch('http://localhost:8080/resend-verification', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email })
      });
      const data = await res.json();
      if (data.status === 'success' || data.message?.toLowerCase().includes('resent')) {
        setStatus('success');
  ...
  return (
    <Box minHeight="100vh" display="flex" alignItems="center" justifyContent="center" sx={{ background: 'radial-gradient(circle at 30% 50%, #a259ff 0%, #181c24 70%)', fontFamily: 'Inter, Arial, sans-serif' }}>
      <Paper elevation={6} sx={{ borderRadius: 3, p: 5, maxWidth: 400, width: '100%' }}>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: 'easeOut' }}>
          <Typography variant="h5" align="center" fontWeight={600} mb={3} color="#23283a">
        ...
        <Box mt={3} textAlign="center" className="footer">
          <Link href="/" underline="hover" color="#4f8cff" fontWeight={500} fontSize="0.98rem">
