...
const ResetPasswordPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.status === 'success' || data.message?.toLowerCase().includes('reset successful')) {
        setStatus('success');
      ...
      <Paper elevation={6} sx={{ borderRadius: 4, p: 5, maxWidth: 410, width: '100%' }}>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: 'easeOut' }}>
          ...
          {token ? (
            ...
            <Typography color="text.secondary" align="center">Invalid or missing reset token.</Typography>
          )}
        </motion.div>
