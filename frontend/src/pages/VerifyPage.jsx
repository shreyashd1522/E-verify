...
const VerifyPage = () => {
  ...
  const handleSubmit = async (e) => {
    if (token) return; // Prevent form if verifying by token
    e.preventDefault();
    setStatus('loading');
    setMessage('');
    try {
          ...
          <AnimatePresence>
            {status === 'loading' && (
              <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <Lottie loop play animationData={loadingAnim} style={{ width: 100, height: 100, margin: '0 auto' }} />
                <Typography color="text.secondary">{token ? 'Verifying your email...' : 'Waiting for email verification...'}</Typography>
              </motion.div>
            )}
            {status === 'success' && (
              <motion.div key="success" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <Lottie loop play animationData={successAnim} style={{ width: 100, height: 100, margin: '0 auto' }} />
                <Typography color="success.main">{message}</Typography>
              </motion.div>
            )}
            {status === 'error' && (
              <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <Lottie loop play animationData={errorAnim} style={{ width: 100, height: 100, margin: '0 auto' }} />
